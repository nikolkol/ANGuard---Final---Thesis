...
CHAT_ID = '1104663066'
LOG_FILE = "/Users/angelinm/Downloads/Binus/Skripsi/notif telegram/modsec.log"
...
DATETIME_REGEX = r"\[(\d{2}/[A-Za-z]{3}/\d{4}:\d{2}:\d{2}:\d{2} [+\-]\d{4})\]"
...
def extract_info(log_entry):
    ...
    log_time = re.search(DATETIME_REGEX, log_entry)

    detection_time = "N/A"
    if log_time:
        try:
            detection_time = datetime.strptime(log_time.group(1), "%d/%b/%Y:%H:%M:%S %z").strftime('%Y-%m-%d %H:%M:%S %z')
        except ValueError:
            print(f"Error parsing datetime: {log_time.group(1)}")
...
Detection Time: {first_entry['detection_time']}
